import java.util.*;

public class Main {

    private final static Map<Integer, Location> locations = new HashMap<>();


    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);
        locations.put(0, new Location(0, "You are sitting in front of a computer learning Java"));
        locations.put(1, new Location(1, "You are standing at the end of a road before a small brick building"));
        locations.put(2, new Location(2, "You are at the top of a hill"));
        locations.put(3, new Location(3, "You are inside a building, a well house for a small spring"));
        locations.put(4, new Location(4, "You are in a valley beside a stream"));
        locations.put(5, new Location(5, "You are in the forest"));

        locations.get(1).addExits("N",5);
        locations.get(1).addExits("E",3);
        locations.get(1).addExits("W",2);
        locations.get(1).addExits("S",4);

        locations.get(2).addExits("W",5);

        locations.get(3).addExits("E",1);

        locations.get(4).addExits("W",2);
        locations.get(4).addExits("N",1);

        locations.get(5).addExits("S",1);
        locations.get(5).addExits("W",2);

        int loc =1;
        System.out.println("Use Upper case in words that indicate direction. 'West' f.e");

        while (true){
            System.out.println(locations.get(loc).getDescription());
            if (loc==0){
                break;
            }

            System.out.println("Available exits...");
            Map<String, Integer> exits= locations.get(loc).getExists();
            for(String exit: exits.keySet()){
                System.out.println(exit+", ");
            }

            String[] direction = scanner.nextLine().split("");
            ArrayList<String> directions = new ArrayList<>(Arrays.asList(direction));

            boolean flag=false;
            for(String i :directions){
                if (exits.containsKey(i)){
                loc=exits.get(i);
                flag=true;
                break;
                }

            }

            if (!flag){
                System.out.println("Exit did not Exist");
                flag=true;
            }

        }
    }
}
